...
module.exports = class WSClient {
  ...
  handleWSMessage (msg) {
    const [type] = msg
    const handler = this._pendingRequests[type] || this._msgHandlers[type]

    if (!handler) {
      console.error('ws_client: handler not found', type)
    }
  ...
  async request (msg, { timeout = 3000, attempts = 1 } = {}) {
    const requestID = nonce()

    try {
      const response = await retry(() => new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => {
          reject(new Error('timeout'))
        }, timeout)

        this._pendingRequests[requestID] = (_, [__,...args]) => {
          clearTimeout(timeoutId)
          resolve(...args)
        }
        this.send([...msg, requestID])
      }), { maxAttempts: attempts, delay: 100 })
      return response
    } finally {
