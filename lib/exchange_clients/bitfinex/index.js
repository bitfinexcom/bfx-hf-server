...
class BitfinexEchangeConnection {
  ...
  initRestAuth (args) {
    ...
    this.restAuth = new RESTv2({ url: this.restURL, apiKey, apiSecret })
  }
  ...
  async getOrderHistory () {
    const request = promisify(this.restAuth.orderHistory).bind(this.restAuth)

    const res = await request(null, null, Date.now(), 10)
    return res
  }
