...
class BitfinexExchangeConnection {
  ...
  async resubscribe () {
    ...
    for (const data of unsubs) {
      const chanID = await this.subscribe(data)
      if (_isFunction(this.subscribedCallback)) {
        this.subscribedCallback(['subscribed', 'bitfinex', chanID, data])
      }
    }
  }
