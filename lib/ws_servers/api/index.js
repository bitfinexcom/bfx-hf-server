...
const onRecurringAlgoOrderSubmit = require('./handlers/on_recurring_algo_order_submit')
const onRecurringAlgoOrderLoad = require('./handlers/on_recurring_algo_order_load')
const onRecurringAlgoOrderLoadList = require('./handlers/on_recurring_algo_order_load_list')
const onRecurringAlgoOrderCancel = require('./handlers/on_recurring_algo_order_cancel')
...
module.exports = class APIWSServer extends WSServer {
  constructor ({
    ...
    super({
      ...
      msgHandlers: {
        ...
        'recurring_algo_order.submit': onRecurringAlgoOrderSubmit,
        'recurring_algo_order.load': onRecurringAlgoOrderLoad,
        'recurring_algo_order.load_list': onRecurringAlgoOrderLoadList,
        'recurring_algo_order.update': onRecurringAlgoOrderUpdate,
        'recurring_algo_order.cancel': onRecurringAlgoOrderCancel,
  ...
  async onWSSConnection (ws) {
    ...
    if (this.metricsServerURL) {
      this._createMetricsClient(session)
    }
  }

  _createMetricsClient (session) {
    const metricsClient = createMetricsClient(this, session)
