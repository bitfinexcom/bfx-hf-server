...
module.exports = class APIWSServer extends WSServer {
  ...
  async onWSSConnection (ws) {
    ...
    ws.getClient = () => {
      if (ws.mode) {
        return ws.services[ws.mode].client
      }
      return null
    }
    ws.getAlgoWorker = () => {
      if (ws.mode) {
        return ws.services[ws.mode].algoWorker
      }
      return null
    }
   ...
   */
  async createAlgoWorker (ws) {
    const tracerDir = path.join(this.dataDir, 'traces')
    const { affiliateCode } = await this.getUserSettings()
