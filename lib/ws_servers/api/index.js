...
module.exports = class APIWSServer extends WSServer {
  ...
  async onWSSConnection (ws) {
    ...
    const getService = (name) => {
      ...
      return services[ws.mode][name]
    }

    /**
     * @typedef {WebSocket} WebSocketContext
     * @property {{key: string?, secret: string?}} bitfinexCredentials
     * @property {'main' | 'paper' | null} mode
     * @property {boolean?} isPaper
     * @property {function(): BitfinexExchangeConnection?} getClient
     * @property {function(): AlgoWorker?} getAlgoWorker
     * @property {function(): DmsRemoteControl?} getDmsControl
     * @property {function(): StrategyManager?} getStrategyManager
     * @property {function(name: string, instance: any)} setService
     * @property {function()} closeServices
     */

    ws.bitfinexCredentials = { key: null, secret: null }
    ws.mode = null
    ws.isPaper = null
    ws.getClient = () => getService('client')
    ws.getAlgoWorker = () => getService('algoWorker')
    ws.getDmsControl = () => getService('dmsControl')
    ws.getStrategyManager = () => getService('strategyManager')
    ws.setService = (name, instance) => {
      if (!ws.mode) throw new Error('mode is not defined')
      const service = services[ws.mode]
      if (!(name in service)) throw new Error(`unrecognized service ${name}`)
      service[name] = instance
    }
    ws.closeServices = () => {
      for (const hub of Object.values(services)) {
        hub.close()
      }
    }

    await this._sendInitialConnectionData(ws)
  }
