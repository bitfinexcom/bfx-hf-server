...
class ConnectionManager {
  ...
  async start (args) {
    ...
    try {
      const { ws, apiKey, apiSecret, dmsScope, mode } = args
      ...
      this._updateCredentials({ mode, apiKey, apiSecret, dmsScope })
      await this._startDmsControl(args)

      if (hasClients && !hasNewCredentials) {
        return
      }
  ...
  _updateCredentials ({ mode, apiKey, apiSecret, dmsScope }) {
    this.mode = mode
