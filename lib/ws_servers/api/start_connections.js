...
class ConnectionManager {
   ...
   * @param {number} packetWDDelay
   * @returns {Promise<*>}
   */
  async updateDms (server, session, dms, dmsScope, packetWDDelay) {
    if (!dmsScope) {
   ...
   */
  async onSettingsUpdate (session, oldSettings, newSettings) {
  // restart ws connections if packet delay setting updated
    if (oldSettings.packetWDDelay !== newSettings.packetWDDelay) {
      await this._restartConnections(session, newSettings)
    }
  }
   ...
   * @private
   */
  async _restartConnections (session, settings) {
    const { packetWDDelay } = settings

    const dmsControl = session.getDmsControl()
    if (dmsControl && dmsControl.isOpen) {
      dmsControl.restartConnection(packetWDDelay)
    }

    const bfxClient = session.getClient()
    if (bfxClient && bfxClient.isOpen) {
      bfxClient.restartConnection(packetWDDelay)
    }

    const algoWorker = session.getAlgoWorker()
    if (algoWorker && algoWorker.isOpen) {
      algoWorker.restartConnection(packetWDDelay)
    }

    const strategyManager = session.getStrategyManager()
    if (strategyManager) {
      const { apiKey, apiSecret } = session.getCredentials()
      await strategyManager.restartConnection(apiKey, apiSecret, packetWDDelay)
    }
  }
  ...
  async _startDmsControl (server, session, settings) {
    ...
    const { dms, packetWDDelay } = settings
    return this.updateDms(server, session, dms, dmsScope, packetWDDelay)
  }
