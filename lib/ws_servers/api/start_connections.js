...
class ConnectionManager {
  ...
  async _startMetricsClient (server, session, ws, shouldUpdate = false) {
    ...
    if (metricsClient && metricsClient.isOpen && !shouldUpdate) {
      return { metricsClient: true }
    }

    metricsClient = await createMetricsClient(server, ws)
    session.setMetricsClient(metricsClient)

    const { apiKey, apiSecret } = session.getCredentials()
    const { dmsScope: scope } = session
    await metricsClient.open({ apiKey, apiSecret, scope })
    return { metricsClient: true }
  }
  ...
  async _startBfxClient (server, session, ws, shouldUpdate = false) {
    ...
    const sendDataToMetricsServerFunc = session.sendDataToMetricsServer()
  ...
  async _startStrategyManager (server, session, ws) {
    ...
    const { dmsScope: scope = 'app' } = session
    const sendDataToMetricsServerFunc = session.sendDataToMetricsServer()

    strategyManager = createStrategyManager(server, ws, scope, sendDataToMetricsServerFunc)
