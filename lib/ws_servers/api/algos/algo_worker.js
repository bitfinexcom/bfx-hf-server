...
class AlgoWorker {
  ...
  async start ({ apiKey, apiSecret, authToken, userId }) {
    ...
    const [authResponse] = await host.connect()
    const { caps } = authResponse

    this.checkPermissions(caps)
  ...
  checkPermissions (caps) {
    const hasAllRequiredPermissions = requiredPermissions.every(perm => _get(caps, perm))

    if (!hasAllRequiredPermissions) {
      throw new Error('The given API key does not have the required permissions, please make sure to enable "get" and "create" capacities for "Orders", and "Wallets"')
    }
  }
}
