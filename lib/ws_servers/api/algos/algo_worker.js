...
class AlgoWorker {
  constructor (settings, algoOrders, bcast, algoDB, logAlgoOpts, marketData, mode) {
    this.host = null
    this.userId = null
  ...
  async sendActiveAlgos () {
    ...
    if (aos.length === 0) {
      this.pub(['data.aos', 'bitfinex', aos])
      return
    }

    const isPaper = this.mode === 'paper' ? 1 : 0
    ...
    this.pub(['data.aos', 'bitfinex', activeAlgoOrders])
  }
