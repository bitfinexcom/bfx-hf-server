...
class AlgoWorker {
  ...
  _registerEvents () {
    ...
    this.host.on('ao:stopped', async (data) => {
      ...
      this._sendSuccess('Stopped algo order')
  ...
  async _runOrder (aoID, ao) {
    try {
      ...
      this._sendSuccess(`Started AO ${name} on Bitfinex`)
