...
class AlgoWorker {
   ...
   * @public
   */
  async sendActiveAlgos () {
    const { mode } = this.settings

    const aos = await this.getActiveAlgosFromDB()
    if (aos.length === 0) {
      this.pub(['data.aos', 'bitfinex', mode, aos])
      return
    }

    const activeAlgoOrders = this._getActiveAlgosByMode(mode, aos)

    this.pub(['data.aos', 'bitfinex', mode, activeAlgoOrders])
  }
  ...
  async sendActiveAlgosForAllModes () {
    const aos = await this.getActiveAlgosFromDB()

    this.pub(['data.aos', 'bitfinex', 'paper', this._getActiveAlgosByMode('paper', aos)])
    this.pub(['data.aos', 'bitfinex', 'main', this._getActiveAlgosByMode('main', aos)])
  }
  ...
  registerEvents () {
    ...
    this.host.on('meta:reload', async () => {
      ...
      await this.sendActiveAlgosForAllModes()
    })
