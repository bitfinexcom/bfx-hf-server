...
const { auth: { tokenTtlInSeconds } } = require('../../../../config/algo_server.conf.json')
...
class AlgoWorker {
  ...
  async start (apiKey, apiSecret, userID) {
    ...
    const authToken = await this.generateAuthToken(restURL, apiKey, apiSecret)
