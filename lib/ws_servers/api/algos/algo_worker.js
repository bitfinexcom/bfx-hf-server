...
} = require('../../../util/validate_credentials')
...
class AlgoWorker {
  ...
  restartConnection (packetWDDelay) {
    if (packetWDDelay) {
      this.host.restartConnection(packetWDDelay)
    }
  }
  ...
  async runOrder (aoID, ao) {
    ...
    try {
      ...
      this.sendSuccess(`Started AO ${alias} on Bitfinex`, [
        'startedAO',
        { alias, target: 'Bitfinex' }
      ])
      this.pub([
        'data.ao',
        'bitfinex',
        mode,
        { id, gid, alias, name, label, args, i18n, createdAt, lastActive }
      ])
      return gid
    } catch (e) {
