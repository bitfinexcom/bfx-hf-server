...
const { getLastActive, getOrderHistory } = require('../../../util/get_last_active')
...
class AlgoWorker {
  ...
  async start ({ apiKey, apiSecret, authToken, userId }) {
    ...
    this.orderHistoryList = await getOrderHistory(restURL, apiKey, apiSecret)
  ...
  async sendActiveAlgos () {
    ...
    const activeAlgoOrders = aos
      ...
      .map(({ gid, createdAt, algoID, state, alias }) => {
        const { args, label, name } = state
        const lastActive = getLastActive(gid, this.orderHistoryList)
