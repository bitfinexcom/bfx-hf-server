...
class AlgoWorker {
  ...
  async start ({ apiKey, apiSecret, authToken, userId }) {
    ...
    try {
      ...
      if (message.includes('permission invalid')) {
        message = 'The given API key does not have the required permissions, please make sure to enable "get" and "create" capacities for "Account", "Orders", and "Wallets"'
      }
