...
class AlgoWorker {
  ...
  async submitOrder ({ aoID, order, gid, state }) {
    ...
    try {
      const [serialized, uiData] = state
        ? await host.loadAO(aoID, gid, state)
        : await host.startAO(aoID, order)

      const { name, label, args, gid: uiGid } = uiData
      d('ao started: %s %s', name, label)

      await this._updateAlgo(serialized)

      this.sendSuccess(`Started AO ${name} on Bitfinex`)
      this.pub(['data.ao', 'bitfinex', { gid: uiGid, name, label, args }])
    } catch (e) {
