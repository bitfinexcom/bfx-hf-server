...
module.exports = async (server, ws, msg) => {
  ...
  if (formSent) {
    const invalidCredsMessage = ['data.api_credentials.validation', {
    ...
    try {
      ...
      if (!isValidAccount) {
        return send(ws, invalidCredsMessage)
      }
    } catch (e) {
      notifyError(ws, 'Invalid API credentials', ['invalidAPICredentials'])
      return send(ws, invalidCredsMessage)
    }
  }
