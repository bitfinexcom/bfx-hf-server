...
const createRecurringAlgoOrder = async (ws, rest, algoDB, payload) => {
  ...
  const { meta = {} } = payload
  const recurringAO = await rest.submitRecurringAlgoOrder(_.omit(payload, ['meta']))
...
const NEW_DUPLICATE_AO_ERROR = 'The recurring order with equal params is active'
...
module.exports = async (server, ws, msg) => {
  ...
  try {
    ...
    if (e.message && e.message.includes(DUPLICATE_AO_ERROR)) {
      return sendError(ws, NEW_DUPLICATE_AO_ERROR, ['algoOrderEqualParamsError'])
    }
