...
const createRecurringAlgoOrder = async (rest, algoDB, order) => {
  ...
  const createdAt = new Date(recurringAO.createdAt).getMilliseconds()
...
module.exports = async (server, ws, msg) => {
  ...
  try {
    const { recurringAlgoOrderId, gid, alias, createdAt, lastActive, algoID } = await createRecurringAlgoOrder(rest, algoDB, payload)
    sendStatus('success', recurringAlgoOrderId)
    send(ws, ['data.ao', 'bitfinex', mode, { id: algoID, gid, alias, name: alias, createdAt, lastActive }])
  } catch (e) {
    sendStatus('failed')
