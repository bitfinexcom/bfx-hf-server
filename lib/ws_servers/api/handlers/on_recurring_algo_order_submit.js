...
const validatePayload = (payload) => {
  const schema = Joi.object({
    alias: Joi.string().optional().default(Recurring.name).label('Alias'),
...
const createRecurringAlgoOrder = async (rest, algoDB, payload) => {
  ...
  const recurringAO = await rest.submitRecurringAlgoOrder(_.omit(validatedPayload, ['meta']))
  ...
  try {
    ...
    send(ws, [
      ...
      `Recurring AO ${alias} was created on Bitfinex`,
      {
        ...
        props: { alias: alias, target: 'Bitfinex' }
      }
    ])
    ...
    send(ws, [
      ...
      'bitfinex',
      mode,
      { id: algoID, label, gid, alias, name, args, createdAt, lastActive }
    ])
  } catch (e) {
    sendStatus('failed')
