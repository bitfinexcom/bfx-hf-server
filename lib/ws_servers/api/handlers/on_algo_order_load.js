...
module.exports = async (server, ws, msg) => {
  ...
  const orderHistoryList = await getOrderHistory(restURL, apiKey, apiSecret)
  ...
  for (const algOrder of orders) {
    ...
    try {
      ...
      const { id, alias, name, label, args, i18n } = uiData

      const lastActive = getLastActive(gid, orderHistoryList)
