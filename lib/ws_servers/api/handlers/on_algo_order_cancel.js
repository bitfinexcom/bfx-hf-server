...
module.exports = async (server, ws, msg) => {
  ...
  const { recurringAlgoOrderId } = await AlgoOrder.get({ gid, algoID: Recurring.id })
  if (recurringAlgoOrderId) {
    return recurringAOCancelHandler(server, ws, [null, authToken, exID, recurringAlgoOrderId, gid])
  }
