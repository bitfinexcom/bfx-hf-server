...
module.exports = async (server, ws, msg) => {
  ...
  if (algoOrder && algoOrder.recurringAlgoOrderId) {
    const alias = JSON.parse(algoOrder.state)?.alias

    return recurringAOCancelHandler(server, ws, { exID, recurringAlgoOrderId: algoOrder.recurringAlgoOrderId, gid, alias })
  }
