...
module.exports = async (server, ws, msg) => {
  const [
    ...
    strategyContent, seedCandleCount, margin, id
  ] = msg
  ...
  const err = validateStrategyArgs({ id, name, symbol, tf, includeTrades, seedCandleCount, margin })
  if (err) {
    sendLiveExecutionSubmitStatus(false)
    return sendError(ws, err)
  }

  const strategyMapKey = `${id}-${symbol}-${tf}`
  ...
  if (!isAuthorized(ws, authToken)) {
    ...
    return sendError(ws, 'Currently running the same strategy. Can\'t execute strategy with same options.')
  }
