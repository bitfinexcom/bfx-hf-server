...
class StrategyManager {
  ...
  async execute (parsedStrategy = {}, strategyOpts = {}) {
    ...
    const abortStrategy = async (mode) => {
      if (mode === ExitModes.CLOSE_AT_MARKET) {
        await closeOpenPositions(strategy)
      } else if (mode === ExitModes.KEEP_ASSETS) {
        await closePendingOrders(strategy)
      }

      await this.close(strategyMapKey)
    }

    this.performanceWatchers = startPerformanceWatchers(perfManager, abortStrategy, strategyOpts)
