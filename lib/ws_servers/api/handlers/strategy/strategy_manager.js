...
class StrategyManager {
  ...
  _registerStrategyExecutionListeners (liveStrategyExecutor, strategyMapKey, strategyOptions) {
    liveStrategyExecutor.on('error', (error) => {
      ...
      if (/minimum size/.test(errorMessage)) {
        return this._handleMinimumSizeError(errorMessage, strategyMapKey, strategyOptions)
      } else if (/balance/.test(errorMessage)) {
        return this._handleInsufficientBalanceError(errorMessage, strategyMapKey, strategyOptions)
      }
    })
   ...
   * @param strategyOpts
   */
  _handleMinimumSizeError (err, strategyMapKey, strategyOptions) {
    ...
    this._sendError(err, strategyMapKey, strategyOptions)
   ...
   * @param strategyOpts
   */
  _handleInsufficientBalanceError (err, strategyMapKey, strategyOptions) {
  ...
  async execute (parsedStrategy = {}, strategyOptions = {}) {
    ...
    this._registerStrategyExecutionListeners(liveStrategyExecutor, gid, strategyOptions)

    await liveStrategyExecutor.execute()

    const startedOn = Date.now()
    strategy.startedOn = startedOn
    ...
    this.pub(['strategy.live_execution_started', gid, this._formatStrategy(strategy, strategyOptions)])
   ...
   */
  getActiveStrategies () {
    const activeStrategies = [...this.strategy.values()]

    return activeStrategies.map(({ strategy, strategyOptions }) => this._formatStrategy(strategy, strategyOptions))
  }
  ...
  async _saveStrategyExecutionResults (strategyMapKey, saveOpts) {
    ...
    const { id: strategyId = '', label, symbol } = strategyOptions
  ...
  _formatStrategy (strategy, strategyOptions) {
    const { gid: id, label, symbol, startedOn } = strategy
    const { id: strategyId } = strategyOptions
