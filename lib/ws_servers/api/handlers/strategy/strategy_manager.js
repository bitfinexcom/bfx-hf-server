...
const { WD_RECONNECT_DELAY, WD_PACKET_DELAY, HB_INTERVAL_MS } = require('../../../../constants')
...
class StrategyManager {
  ...
  async restartConnection (apiKey, apiSecret, packetWDDelay) {
    this.packetWDDelay = packetWDDelay
    this._disconnect()
    await this.start({ apiKey, apiSecret })

    // restart execution of all active strategies with new ws2Manager instance
    for (const [, activeStrategy] of this.strategy) {
      await activeStrategy.liveStrategyExecutor.restartExecution(this.ws2Manager)
    }
  }
  ...
  async _connect () {
    ...
    this.ws2Manager.onWS('message', {}, this._onMessage.bind(this))
  ...
  // send heartbeat to the server to monitor connection status
    sendHB(this.ws2Manager)
  }

  _disconnect () {
    if (this.ws2Manager) this.ws2Manager.closeAllSockets()
  ...
  _onWSClose () {
    ...
    // check if packet delay timedout
    if (this._hasPacketDelayTimedout()) {
  ...
  _onWSOpen () {
    ...
    // check if packet delay timedout
    if (this.connectionLost && this._hasPacketDelayTimedout()) {
      this.connectionLost = false
   ...
   * @param {boolean} [strategyOptions.margin]
   * @param {string} [strategyOptions.strategyType]
   * @param {boolean} [strategyOptions.isDerivative]
   * @param {string} [strategyOptions.baseCurrency]
   * @param {string} [strategyOptions.quoteCurrency]
   * @param {number} [strategyOptions.maxLeverage]
   ...
   * @param {boolean} [strategyOptions.useMaxLeverage]
   * @param {boolean} [strategyOptions.increaseLeverage]
   * @param {number} [strategyOptions.leverage]
   * @param {boolean} [strategyOptions.addStopOrder]
   * @param {number} [strategyOptions.stopOrderPercent]
   */
