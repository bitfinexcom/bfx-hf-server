...
const CLOSE_CONNECTIONS_DELAY = 1 * 60 * 1000 // two minutes
...
class StrategyManager {
  constructor (settings, bcast, strategyExecutionDB) {
    ...
    this.closeConnectionsDelay = CLOSE_CONNECTIONS_DELAY
  ...
  _unsubscribeChannels (channels, strategyMapKey) {
    ...
    const unsubscribeTrades = tradeChannelIndex !== -1
  ...
  async _saveStrategyExecutionResults (strategyMapKey, saveOpts) {
    ...
    const { id: strategyId = '', name, symbol } = strategyOpts
  ...
  async close (strategyMapKey) {
    ...
    this.pub(['strategy.live_execution_results', strategyExecutionResults])
