...
module.exports = async (server, session, msg) => {
  ...
  for (const [mode, { algoWorker }] of Object.entries(session.services)) {
    if (algoWorker) {
      tasks.push(...payload[mode].remove.map(({ gid }) => algoWorker.cancelOrder(gid)))

      const activeAOs = algoWorker.getActiveAlgosFromHost()

      for (const { algoID, gid } of payload[mode].resume) {
        const instance = activeAOs.find(({ state }) => state.gid === gid && state.id === algoID)
        const { state = {} } = instance
        const { createdAt, active } = state

        if (!state || active) {
          continue
        }

        tasks.push(algoWorker.host.loadAO(algoID, gid, state, createdAt))
      }
    }
  }
