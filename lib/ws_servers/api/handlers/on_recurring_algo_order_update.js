...
const updateRecurringAlgoOrder = async (rest, algoDB, gid, order) => {
  ...
  order.algoOrderId = algoOrder.recurringAlgoOrderId

  validatePayload(order)

  let mappedRecurringAO
  const result = await rest.updateRecurringAlgoOrder(order)

  if (result === 1) {
    const updatedRecurringAO = await rest.getRecurringAlgoOrder(order.algoOrderId)
    mappedRecurringAO = mapRecurringAlgoOrderState(updatedRecurringAO)

    await AlgoOrder.update({ gid, algoID: Recurring.id }, {
      ...mappedRecurringAO,
