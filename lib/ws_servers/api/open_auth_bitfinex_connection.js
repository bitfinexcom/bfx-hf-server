...
const {
  ...
  DMS_ENABLED,
  PAPER_MODE_PAIRS,
  PAPER_MODE_CURRENCIES
} = require('../../constants')

const isValidPair = (isPaper) => {
  return ([pair]) => {
    return !isPaper || PAPER_MODE_PAIRS.includes(pair)
  }
}

const isValidCurrency = (isPaper) => {
  return ([curr]) => {
    return !isPaper || PAPER_MODE_CURRENCIES.has(curr)
  }
}

module.exports = ({ ws, apiKey, apiSecret, authToken, dms, d, wsURL, restURL, isPaper }) => {
  ...
  const checkPair = isValidPair(isPaper)
  const checkCurrency = isValidCurrency(isPaper)
  ...
  client.onData(payload => {
    ...
    console.log('#', msgType, JSON.stringify(msgData))

    switch (msgType) {
      case 'ws': {
        const filteredData = msgData.filter(checkCurrency)
        return send(ws, ['data.balances', 'bitfinex', filteredData])
      }
      case 'wu': {
        if (!checkCurrency(msgData)) return
        return send(ws, ['data.balance', 'bitfinex', msgData])
      }
      case 'ps': {
        const filteredData = msgData.filter(checkPair)
        return send(ws, ['data.positions', 'bitfinex', filteredData])
      }
      case 'pu': {
        if (!checkPair(msgData)) return
      ...
      case 'os': {
        return send(ws, ['data.orders', 'bitfinex', msgData])
      }
      case 'on': {
