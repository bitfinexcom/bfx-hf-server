...
module.exports = ({ ws, apiKey, apiSecret, authToken, dms, d, wsURL, restURL, isPaper, scope = 'app' }) => {
  ...
  client.onData(payload => {
    ...
    switch (msgType) {
      ...
      case 'os': {
        return onOrdersSnapshot(ws, isPaper, scope, msgData)
      }
      case 'on': {
        return onOrderNew(ws, isPaper, scope, msgData)
      }
      case 'ou': {
        return onOrderUpdate(ws, isPaper, scope, msgData)
      }
      case 'oc': {
        return onOrderClose(ws, isPaper, scope, msgData)
      }
      case 'n': {
