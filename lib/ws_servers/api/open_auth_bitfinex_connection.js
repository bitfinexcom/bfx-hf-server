...
module.exports = ({ ws, apiKey, apiSecret, authToken, dms, d, wsURL, restURL, isPaper }) => {
  ...
  client.onData(payload => {
    ...
    switch (msgType) {
      case 'ws': {
        const filteredData = msgData.filter(item => !isPaper || PAPER_MODE_CURRENCIES.has(item[0]))
        return send(ws, ['data.balances', 'bitfinex', filteredData])
      }
      case 'wu': {
        if (isPaper && !PAPER_MODE_CURRENCIES.has(msgData[0])) {
          return
        }
        return send(ws, ['data.balance', 'bitfinex', msgData])
      }
      case 'ps': {
        if (isPaper && !PAPER_MODE_PAIRS.includes(msgData[0])) {
          return
        }
