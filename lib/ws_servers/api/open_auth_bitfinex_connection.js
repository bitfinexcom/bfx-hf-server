...
module.exports = ({ ws, apiKey, apiSecret, authToken, dms, d, wsURL, restURL, isPaper, dmsScope = 'app', sendDataToMetricsServer, mode }) => {
  ...
  client.onData(payload => {
    ...
    switch (msgType) {
      ...
      case 'on': {
        return onOrderNew(ws, isPaper, dmsScope, msgData, sendDataToMetricsServer)
      }
      case 'ou': {
        return onOrderUpdate(ws, isPaper, dmsScope, msgData, sendDataToMetricsServer)
      }
      case 'oc': {
        return onOrderClose(ws, isPaper, dmsScope, msgData, sendDataToMetricsServer)
      }
      case 'n': {
