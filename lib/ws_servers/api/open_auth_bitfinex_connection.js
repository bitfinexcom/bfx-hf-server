...
module.exports = ({ ws, apiKey, apiSecret, authToken, dms, d, wsURL, restURL, isPaper, dmsScope = 'app', sendDataToMetricsServer, mode, session, packetWDDelay }) => {
  notifyInfo(ws, 'Connecting to Bitfinex...', ['connectingTo', { target: 'Bitfinex' }])

  const client = new BitfinexExchangeConnection({ wsURL, restURL, packetWDDelay })
